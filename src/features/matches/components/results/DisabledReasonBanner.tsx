import React from 'react'
import { formatLocalTime } from '../../../../shared/utils/datetime'

type Props = {
  reason: string
  endAt?: string
}

export default function DisabledReasonBanner({ reason, endAt }: Props) {
  return (
    <div className="bg-yellow-900/20 border border-yellow-600/50 rounded-lg p-3 mb-4">
      <div className="flex items-start gap-2">
        <svg className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
        </svg>
        <div className="flex-1">
          <p className="text-sm text-yellow-200 font-medium mb-1">
            {endAt 
              ? `Disponible cuando inicie el partido (${formatLocalTime(endAt)})`
              : 'No disponible aún'
            }
          </p>
          <p className="text-xs text-yellow-300/80">{reason}</p>
        </div>
      </div>
    </div>
  )
}
